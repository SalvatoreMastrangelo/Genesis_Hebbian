...
#
# A WP2 experiment lives in its OWN FOLDER and contains three files:
#   - run.yaml         (HebbianEvolutionConfig — checkpoint paths can stay blank)
...
#
# Examples:
#   bash run_parallel.sh                                     # uses src/WP2/experiments/
#   bash run_parallel.sh src/WP2/experiments/batch_0/        # explicit batch
#   bash run_parallel.sh src/WP2/experiments/batch_0/ --gpus=2
#
...
BATCH_DIR="${1:-${REPO_ROOT}/src/WP2/experiments}"
shift || true
...
for EXP_DIR in "${EXP_DIRS[@]}"; do
  ...
  # --- Path relative to src/WP2 (what train.slurm expects in EXP_REL) ---
  EXP_REL="$(realpath --relative-to="${REPO_ROOT}/src/WP2" "${EXP_DIR}")"
