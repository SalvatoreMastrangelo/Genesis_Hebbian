...
The third level is the interface between the simulator and the controller: the observations are corrupted by sensor noise, the commands reach the servomotors with a random latency of up to one control step and with a small random offset, and the forest and the initial conditions are drawn at random for every flight. These belong to the definition of the task rather than to the simulator; their values are given with the training of the controllers (\autoref{subsec:training-environment}).
...
\paragraph{End of a flight.} A flight ends when the drone hits a trunk, leaves the corridor sideways, descends to the ground, exceeds a quarter of a turn in roll, pitch or yaw (it has tumbled, or turned back), flies past the end of the forest, or reaches a time limit. The distance $\Delta x$ covered along the corridor, from the point of release to the point where the flight ends, is its \emph{progress}, the first of the two quantities by which bodies are judged in this thesis: a flight that ends at the first tree has a progress of 30~m; the second is the energy spent per metre.
